...
function isValidIpAddress(ipchars: string): boolean {
...
function isInNet(ipaddr: string, pattern: string, maskstr: string): boolean {
...
function shExpMatch(url: string, pattern: string): boolean {
...
function weekdayRange(...args: (string | number)[]): boolean {
...
function dateRange(...args: (string | number)[]): boolean {
...
function timeRange(...args: (string | number)[]): boolean {
	let argc = args.length;
	const date = new Date();
	let isGMT = false;
	if (argc < 1) {
